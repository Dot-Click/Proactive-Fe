...
// Public GET endpoint - anyone can access
export const useGoogleReviews = () => {
    return useQuery({
        queryKey: ["google-reviews"],
        queryFn: async () => {
            try {
                const response = await api.get("/api/user/google-reviews");
                const payload = response.data;
                if (!payload) return [];
...
// Admin-only GET endpoint (if you still need admin-specific features)
export const useAdminGoogleReviews = () => {
    return useQuery({
        queryKey: ["admin-google-reviews"],
        queryFn: async () => {
...
// Protected admin operations (unchanged)
export const useCreateGoogleReview = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async (formData: FormData) => {
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["google-reviews"] }); // Also invalidate public query
            queryClient.invalidateQueries({ queryKey: ["admin-google-reviews"] });
            toast.success("Review created successfully");
        },
        onError: (error: any) => {
