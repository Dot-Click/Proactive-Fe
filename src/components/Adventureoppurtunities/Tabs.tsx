...
const Tabs = ({ activeTab, onTabChange }: TabsProps) => {
  ...
  const tabConfig: { id: TabId; label: string; count: number }[] = [
    { id: "all", label: t("trips.tabs.all"), count: counts.all },
    { id: "open", label: t("trips.tabs.open"), count: counts.open },
    { id: "coming-soon", label: t("trips.tabs.comingSoon"), count: counts.comingSoon },
    { id: "closed", label: t("trips.tabs.closed"), count: counts.closed },
  ];
    ...
    <>
      <div className="px-4 sm:px-16 py-6">
        <div className="bg-[#FFFFFF] border border-[#D9D9D9] shadow-md rounded-full px-6 py-2 w-fit">
          <div
            className="flex flex-wrap lg:gap-8 gap-2 items-center justify-center"
            role="tablist"
          >
            {tabConfig.map((tab) => {
