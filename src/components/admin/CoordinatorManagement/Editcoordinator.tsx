...
const Editcoordinator = () => {
    ...
    const onSubmit = (values: z.infer<typeof formSchema>) => {
        ...
        // Create FormData object with FLAT structure (not nested JSON)
        const formData = new FormData();

        // Append each field individually to FormData (not as JSON string)
        formData.append("fullName", values.username.trim());
        formData.append("phoneNumber", values.phone.replace(/\D/g, ''));
        ...
        // Handle arrays - send as JSON strings since FormData can't send arrays directly
        formData.append("specialities", JSON.stringify(values.Specialties));
